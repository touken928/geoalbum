...
const ErrorToast: React.FC = () => {
  ...
  const getBackgroundColor = (type: AppError['type']) => {
    switch (type) {
      case 'success':
    ...
    <div className="fixed top-4 right-4 z-50 space-y-2 max-w-sm">
      {errors.map((error) => (
        <div
          key={error.id}
          className={`
            ${getBackgroundColor(error.type)}
            ${getTextColor(error.type)}
            border rounded-lg p-4 shadow-lg animate-in slide-in-from-right duration-300
          `}
        >
